...
func createRecords(client dme.Client, zone string, records []libdns.Record) ([]libdns.Record, error) {
	...
	if err == nil {
		fmt.Printf("createRecords input records %s\n", recordJson)
	}
	...
	if err != nil {
		fmt.Printf("createRecords jsonMarshal error %s\n", err)
		return nil, err
	}
	...
	if err != nil {
		fmt.Printf("createRecords johnk error: %s\n", err)
		return nil, err
	}
	...
	newJson, err := json.Marshal(newRecords)
	if err != nil {
		fmt.Printf("createRecords jsonMarshal error %s\n", err)
		return nil, err
	}
